      ...
      {step === 1 ? (
        ...
        <form onSubmit={handleStep2} className="flex flex-col gap-3">
          ...
          <select required className={`h-11 rounded-lg px-4 text-sm border ${isDark ? "bg-white/10 border-white/20 text-white" : "bg-foreground/5 border-border text-foreground"}`}>
            <option value="">Monthly revenue range</option><option>Under OMR 5K</option><option>OMR 5K – OMR 20K</option><option>OMR 20K – OMR 100K</option><option>OMR 100K+</option>
          </select>
  ...
  return (
    <>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial="hidden" animate="visible" variants={stagger} className="space-y-6">
              ...
              <motion.h1 variants={fadeUp} className="text-4xl md:text-5xl lg:text-[3.4rem] font-extrabold leading-[1.1] text-white">
                تطوير Shopify لـ{" "}<span className="bg-gradient-to-r from-[hsl(145,50%,40%)] to-[hsl(165,60%,45%)] bg-clip-text text-transparent">السوق العماني</span>
              </motion.h1>
              <motion.p variants={fadeUp} className="text-lg text-white/70 max-w-xl leading-relaxed" data-speakable>
                CPA-compliant Shopify stores for Oman. OmanNet, Thawani payments, Arabic/English support, and GCC expansion.
              </motion.p>
              <motion.div variants={fadeUp} className="flex flex-wrap gap-4 text-sm text-white/60">
                {["OmanNet & Thawani", "CPA Compliant", "Arabic RTL", "Oman Vision 2040"].map((t) => (<span key={t} className="flex items-center gap-1.5"><CheckCircle className="w-4 h-4 text-[hsl(145,50%,40%)]" /> {t}</span>))}
              </motion.div>
              <motion.div variants={fadeUp}><InlineLeadForm id="hero-om" variant="dark" /></motion.div>
              <motion.p variants={fadeUp} className="text-xs text-[hsl(145,50%,40%)]/80 flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /> Limited free audits for Omani brands this month</motion.p>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4, duration: 0.7 }} className="hidden lg:grid grid-cols-2 gap-4">
              {[
                { value: "OMR 2B+", label: "Oman E-Commerce Market", icon: TrendingUp },
                { value: "95%", label: "Internet Penetration", icon: BarChart3 },
                { value: "74%", label: "Mobile Commerce Share", icon: Rocket },
                { value: "Fast Growing", label: "GCC Digital Economy", icon: Zap },
              ].map((stat) => (
      ...
      <section className="bg-card border-b border-border py-6"><div className="container mx-auto px-6"><div className="flex flex-wrap justify-center items-center gap-8 md:gap-12">
        {[{ val: "300+", label: "Stores Launched" }, { val: "OMR 5M+", label: "Client Revenue" }, { val: "GCC", label: "Expertise" }, { val: "4.9/5", label: "Rating" }, { val: "100%", label: "CPA Ready" }].map((s) => (<div key={s.label} className="text-center"><p className="text-xl md:text-2xl font-bold text-primary">{s.val}</p><p className="text-xs text-muted mt-0.5">{s.label}</p></div>))}
      </div></div></section>
      ...
      <section className="py-16 md:py-24 bg-background"><div className="container mx-auto px-6">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="text-center max-w-3xl mx-auto mb-12">
          ...
          <motion.h2 variants={fadeUp} className="text-3xl md:text-4xl font-bold text-foreground">Why Shopify for Oman E-Commerce</motion.h2>
        </motion.div>
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[
            { icon: CreditCard, title: "OmanNet & Thawani", desc: "OmanNet debit, Thawani Pay, Apple Pay — all Omani payment methods integrated." },
            { icon: Lock, title: "CPA Compliance", desc: "Consumer Protection Authority compliance for e-commerce in Oman." },
            { icon: FileCheck, title: "Arabic RTL Storefront", desc: "Bilingual Arabic/English with RTL layout and Omani localization." },
            { icon: Globe, title: "GCC Expansion", desc: "Expand from Oman to KSA, UAE with multi-currency and regional shipping." },
            { icon: Truck, title: "Oman Post & Aramex", desc: "Oman Post, Aramex, and local couriers with COD and tracking." },
            { icon: Layers, title: "ERP Integration", desc: "Connect with Omani accounting and ERP systems." },
          ].map((item) => (
      ...
      <section className="py-16 md:py-24 bg-card border-y border-border"><div className="container mx-auto px-6">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="text-center max-w-3xl mx-auto mb-14">
          ...
          <motion.h2 variants={fadeUp} className="text-3xl md:text-4xl font-bold text-foreground">Shopify Services for Oman</motion.h2>
        </motion.div>
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[
            { icon: ShoppingCart, title: "Shopify Store Launch", desc: "Premium Oman-market stores with Thawani and Arabic RTL.", link: "/shopify-themes" },
            { icon: Rocket, title: "Shopify Plus Oman", desc: "Enterprise commerce for Omani brands.", link: "/shopify-plus" },
            { icon: RefreshCw, title: "Platform Migration", desc: "Zero-downtime migration from any platform." },
            { icon: BarChart3, title: "CRO & Optimization", desc: "Conversion optimization for Omani shoppers." },
            { icon: Palette, title: "Custom Arabic UX", desc: "Oman-market UX with RTL and trust signals." },
      ...
      <section className="py-14 md:py-20 bg-gradient-to-r from-primary to-[hsl(145,50%,35%)]"><div className="container mx-auto px-6 text-center">
        <h2 className="text-2xl md:text-3xl font-bold text-primary-foreground mb-3">Ready to Scale in Oman?</h2>
        <p className="text-primary-foreground/80 mb-8 max-w-lg mx-auto">Free Shopify growth plan for the Oman market.</p>
        <div className="flex justify-center"><InlineLeadForm id="mid-om" variant="dark" /></div>
      </div></section>

      <ShopifyPricingPackages currency="OMR" prices={["300", "1,200", "3,000"]} headlinePrice="OMR 300"
        descOverrides={{ 0: "Quick-launch store — ideal for startups entering the Omani market." }}
        featureOverrides={{ "Payment Gateway (PayTabs / Telr)": "Payment Gateway (OmanNet / Thawani)", "Local UAE Shipping Setup": "Local Oman Shipping (Oman Post / Aramex)", "Multi-Currency Support (AED, USD, EUR)": "Multi-Currency Support (OMR, USD, AED)", "Multi-Language (English / Arabic)": "Multi-Language (Arabic / English)" }}
      />

      <section className="py-16 md:py-24 section-dark"><div className="container mx-auto px-6">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="text-center max-w-3xl mx-auto mb-14">
          <motion.p variants={fadeUp} className="text-sm font-bold tracking-[0.2em] uppercase text-primary mb-3">Oman Challenges</motion.p>
          <motion.h2 variants={fadeUp} className="text-3xl md:text-4xl font-bold text-white">Challenges We Solve for Omani Brands</motion.h2>
        </motion.div>
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
          {[
            { title: "Thawani & OmanNet Integration", desc: "Omani payment gateways require specialized integration. We handle all local payment methods seamlessly." },
            { title: "Arabic RTL Experience", desc: "Native Arabic UX with proper RTL and Omani cultural design patterns." },
            { title: "Last-Mile Delivery", desc: "Oman's geography requires integration with Oman Post and regional couriers." },
            { title: "CPA Regulations", desc: "Full Consumer Protection Authority compliance for Omani e-commerce." },
          ].map((c) => (
      ...
      <section className="py-16 md:py-24 bg-card border-y border-border"><div className="container mx-auto px-6 max-w-3xl">
        ...
        <Accordion type="single" collapsible className="space-y-3">
          {[
            { q: "Can you integrate Thawani and OmanNet?", a: "Yes. Full OmanNet, Thawani, and Apple Pay integration." },
            { q: "How much does Shopify cost in Oman?", a: "From OMR 300 starter to OMR 3,000+ enterprise." },
            { q: "Is Shopify CPA-compliant?", a: "Yes, with our compliance setup for Omani regulations." },
            { q: "Do you build Arabic stores?", a: "Yes. Full RTL Arabic/English bilingual stores." },
            { q: "Which shipping carriers?", a: "Oman Post, Aramex, and local couriers." },
      ...
      <section className="py-20 md:py-28 section-dark relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="max-w-2xl mx-auto">
            <motion.h2 variants={fadeUp} className="text-3xl md:text-4xl font-bold text-white mb-4">ابدأ نجاحك مع Shopify في عمان</motion.h2>
            <motion.p variants={fadeUp} className="text-white/60 mb-8">Get your free Shopify growth plan for Oman.</motion.p>
            <motion.div variants={fadeUp} className="flex justify-center mb-6"><InlineLeadForm id="final-om" variant="dark" /></motion.div>
            <motion.div variants={fadeUp} className="flex flex-wrap justify-center gap-4 mt-6">
              ...
              <a href="https://wa.me/919544086877?text=Hi%20Globify%2C%20I%27m%20interested%20in%20Shopify%20development%20for%20Oman." target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full px-6 py-2.5 bg-[#25D366] text-white text-sm font-semibold hover:bg-[#22c55e] transition-colors"><MessageCircle className="w-4 h-4" /> WhatsApp Us</a>
            </motion.div>
          </motion.div>
        </div>
      </section>
