    ...
    <>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial="hidden" animate="visible" variants={stagger} className="space-y-6">
              <motion.div variants={fadeUp} className="inline-flex items-center gap-2 bg-white border border-border/20 rounded-full px-4 py-1.5 text-xs font-medium text-foreground shadow-sm">
                Shopify Experts in Spain — Globify
              </motion.div>
              <motion.h1 variants={fadeUp} className="text-4xl md:text-5xl lg:text-[3.4rem] font-extrabold leading-[1.1] text-white">
                Scale Your <span className="bg-gradient-to-r from-primary to-orange-400 bg-clip-text text-transparent">Shopify Store</span> in Spain
              </motion.h1>
              <motion.p variants={fadeUp} className="text-lg text-white/70 max-w-xl leading-relaxed">
                Unlock the massive potential of the Spain market. We build high-converting Shopify stores with local payment gateways (Bizum), logistics, and optimization.
              </motion.p>
              <motion.div variants={fadeUp} className="flex flex-wrap gap-4 text-sm text-white/60">
                {["Bizum Integration","Local Logistics","Spanish SEO","GDPR Compliant"].map((t) => (
                  <span key={t} className="flex items-center gap-1.5"><CheckCircle className="w-4 h-4 text-primary" /> {t}</span>
                ))}
              </motion.div>
              <motion.div variants={fadeUp}><InlineLeadForm id="hero-spain" variant="dark" /></motion.div>
            </motion.div>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.4, duration: 0.7 }} className="hidden lg:block relative">
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8 relative overflow-hidden group hover:bg-white/10 transition-colors">
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                    <h3 className="text-white font-bold">Spain Market Leader</h3>
                    <p className="text-white/40 text-sm">Serving the entire region</p>
                  </div>
                </div>
                <div className="space-y-4">
                  {[
                    { icon: CreditCard, title: "Local Payments", desc: "Bizum, Redsys, PayPal." },
                    { icon: Truck, title: "Logistics Setup", desc: "Correos, SEUR, MRW integration." },
      ...
      <section className="py-16 md:py-24 bg-background">
        <div className="container mx-auto px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="text-center max-w-3xl mx-auto mb-16">
            <motion.p variants={fadeUp} className="text-sm font-bold tracking-[0.2em] uppercase text-primary mb-3">Why Shopify in Spain?</motion.p>
            <motion.h2 variants={fadeUp} className="text-3xl md:text-4xl font-bold text-foreground">Dominating the Spain E-commerce Market</motion.h2>
          </motion.div>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: Globe, title: "Local Payment Gateways", desc: `Native integration for ${'Bizum, Redsys, PayPal'}, which is essential for maximizing conversion rates.` },
              ...
              { icon: Truck, title: "Logistics Ops", desc: `Automated integrations with ${'Correos, SEUR, MRW'} for streamlined last-mile delivery.` },
      ...
      <section className="py-14 md:py-20 bg-gradient-to-r from-primary to-green-600">
        <div className="container mx-auto px-6 text-center text-white">
          <h2 className="text-2xl md:text-4xl font-bold mb-4">Ready to Grow Your Business in Spain?</h2>
          ...
          <div className="flex justify-center"><InlineLeadForm id="mid-spain" variant="dark" /></div>
        </div>
      </section>
      ...
      <section className="py-16 md:py-24 bg-card border-y border-border">
        <div className="container mx-auto px-6 max-w-3xl">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="text-center mb-12">
            ...
            <motion.h2 variants={fadeUp} className="text-3xl md:text-4xl font-bold text-foreground">Shopify in Spain: FAQ</motion.h2>
          </motion.div>
          <Accordion type="single" collapsible className="space-y-3">
            {[
              { q: `Is Shopify suitable for businesses in ${'Spain'}?`, a: `Absolutely. Shopify is globally recognized and fully supports local currencies (${'€'}), local payment gateways, and regional shipping providers.` },
              { q: `How do I accept payments via ${'Bizum'} on Shopify?`, a: "We handle the entire technical integration process to ensure seamless and secure transactions for your customers." },
              ...
              { q: `Which shipping companies work with Shopify in ${'Spain'}?`, a: `Major providers like ${'Correos, SEUR, MRW'} have robust Shopify integrations.` },
            ].map((faq, i) => (
      ...
      <section className="py-20 md:py-28 section-dark relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="max-w-2xl mx-auto">
            ...
            <motion.p variants={fadeUp} className="text-white/60 mb-10">Our Spain-focused Shopify team is here to help you scale.</motion.p>
            <motion.div variants={fadeUp} className="flex justify-center mb-8"><InlineLeadForm id="final-spain" variant="dark" /></motion.div>
